...
int http_read_line(int fd, char *buf,int bufsize)
{
   size_t i = 0;
    for (;;)
    {
...
const char *http_request_line(int fd, char *reqpath, size_t reqpath_len, char *env, size_t *env_len)
{
    ...
    warnx("inside http_request_line");
    ...
    *sp2 = '\0';
    ...
    if(strlen(sp2)+16 > (sizeof(buf) - (envp-env+1)))
    {
        return "Server Protocol too long";
    }
    ...
    {
        *qp = '\0';
        if(strlen(qp+1)+13 > (sizeof(buf)-(envp-env+1)))
        {
    ...
    if(strlen(reqpath)+12 > (sizeof(buf)-(envp-env+1)))
    {
        return "Request URI too long";
    }
    envp += sprintf(envp, "REQUEST_URI=%s", reqpath) + 1;

    if(24 > (sizeof(buf)-(envp-env+1)))
    {
    ...
    *env_len = envp - env + 1;
    ...
    if(*env_len > sizeof(buf))
    {
       return "Environment string too long";
    }
    return NULL;
}
